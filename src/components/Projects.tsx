...
const Projects = ({ disableSectionHeight = false }: SectionProps) => {
  ...
  const cardBase = "w-full max-w-[360px] aspect-[4/3] p-[20px] flex flex-col bg-cover bg-center rounded-[10px] text-[18px] hover:scale-[1.05] transition-all cursor-pointer duration-300";
  ...
  return (
    <section id="projects" style={disableSectionHeight ? undefined : { height: sectionHeight }} className="relative bg-[#111a24] overflow-hidden h-full">
      <div
      ...
      <div className={`absolute inset-0 z-20 flex items-center justify-center transition-opacity duration-300 pointer-events-none`} style={{ opacity: hoverInfo ? 1 : 0 }}>
        {hoverInfo && (
          <div className="text-center">
            ...
            <p className="mt-4 text-2xl bg-gradient-to-r from-[#A8F00E] via-[#CCF576] to-[#D4F8D3] bg-clip-text text-transparent drop-shadow-[0_2px_8px_rgba(0,0,0,0.35)]">{hoverInfo.subtitle}</p>
          </div>
        )}
      </div>

      <Container className="relative z-10 flex flex-col items-center text-white">
        <h2 className="font-bold text-white text-3xl xs:text-4xl lg:text-6xl py-4 lg:py-6">PROJECTS</h2>
